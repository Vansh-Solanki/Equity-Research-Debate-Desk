...
- Status: **Done** — `rag/` (chunker, embedder, chroma_store, reranker, retriever) chunks a filing structure-aware (splits into Item-N sections first via `deep_dive/section_splitter.py`, then sentence-safe sub-chunks within each section, ~1000 chars with 1 sentence of overlap — reworked from the original fixed-window/word-boundary approach), embeds locally via `all-MiniLM-L6-v2`, stores per-company in a persistent Chroma collection with real section labels, and reranks candidates with `cross-encoder/ms-marco-MiniLM-L-6-v2`. `scripts/test_phase5_retrieval.py` scored 9/10 on AAPL and 10/10 on MSFT (needs >= 8/10) — see progress.md for the real bugs this surfaced across both the original build (a stale-duplicate-chunk indexing bug, a page-footer noise issue) and the later chunker rework (an overlap-reseed sizing bug, a fallback-split abbreviation bug, a redundant-reindexing efficiency issue between the main and deep-dive indexes, and a misdiagnosed test-harness sampling-offset bug in `evaluation/retrieval_metrics.py` that was the actual root cause of the original mid-word-snippet symptom) and how they were fixed.
...
- Status: **Done** — `evaluation/` (claim_extractor, entailment_checker, pipeline, metrics, test_set.json) implements extract -> retrieve -> classify, applied uniformly via `evaluation.pipeline.check_statement`, now wired into `orchestration/debate_loop.py` for every Bull/Bear/Judge statement. `scripts/test_phase6_evaluation.py` measures **F1 = 0.72** on a 30-claim hand-labeled test set built from real AAPL/MSFT filing content (re-tuned `DEFAULT_TOP_K` to 3 after Phase 5's chunker rework cleaned up the input; originally 0.68 at `DEFAULT_TOP_K = 2`) — above the 0.65 bar spec.md's acceptance criteria explicitly allow adjusting to once real numbers came in (original target 0.75; see progress.md for the full tuning history, a later claim-extraction fix that splits multi-fact sentences, and why a fully local, un-fine-tuned NLI model has a real ceiling on noisy 10-K prose).
...
- Status: **Done** — `evaluation/dashboard.py` assembles all six metrics from modules each already built for an earlier phase (only debate engagement scoring, `evaluation/engagement_scorer.py`, is new — nothing scored that before). `agents/llm.py` gained a process-global call log so latency/call-count can be measured around a debate run without instrumenting every agent. `scripts/test_phase8_dashboard.py` on AAPL: a real 3-round debate + a deep-dive spawn produced all six numbers in one run (206.8s, 14 LLM calls) — acceptance criterion met. See progress.md for the actual measured numbers and one open discrepancy (Judge-accuracy F1 measured 0.62 here vs Phase 6's originally-recorded 0.68 on the same test set, since resolved as a stale-comparison artifact — see progress.md). **Sept 8 session addition**: two more metrics were added on top of the original six, completing a three-layer Judge-faithfulness framework (`evaluation/judge_faithfulness.py`) scoped after the claim-extractor's meta-commentary fix exposed that groundedness alone can't confirm the Judge's output is trustworthy. Metric 7, verdict consistency (Layer 3, deterministic, no extra LLM call) — checks whether `stronger_side` follows from the real per-side support rate; a live run caught a real case of the Judge saying "tie" on a 79%/44% split. Metric 8, citation accuracy (Layer 2, one small LLM call per citation sentence plus embedding-similarity matching against `rag/embedder`) — checks whether the Judge's memo accurately describes what each side said and its real status, tuned against a new hand-labeled `evaluation/judge_citation_test_set.json` (9/9 after fixing an untested threshold guess). Both verified live — see progress.md's Sept 8 write-up.
